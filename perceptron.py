...
class HiddenNeuron:
    ...
    def count_stochastic_error(self, next_neuron):  # TODO change this method to calculate error of neuron inside it without summing previous neurons error
        self.error += (next_neuron.error * next_neuron.weights[self.number_in_layer]) * sigmoid_derivative(self.value)
...
class ResultNeuron(HiddenNeuron):
    ...
    def count_stochastic_error(self, answer):  # TODO change this method to calculate error of neuron inside it without summing previous neurons error
            ...
            for hidden_neuron in self.hidden_layers_arr[hidden_layer]:
                if hidden_layer == len(self.hidden_layers_arr) - 1:
                    for sinaps in self.result_neurons_arr:
                        hidden_neuron.count_stochastic_error(sinaps)
                else:
